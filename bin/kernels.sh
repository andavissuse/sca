...
# for features corresponding to the running and/or installed
# kernels.
#
...
unameSectionLine=`cat $basicEnvFile | grep -n "^# /bin/uname -a$" | cut -d":" -f1`
	[ $DEBUG ] && echo "*** DEBUG: $0: unameSectionLine: $unameSectionLine"
kernelVer=`sed "$((${unameSectionLine}+1))q;d" $basicEnvFile | cut -d" " -f3`
if ! echo "$kernelVer" | grep -q "^[0-9]"; then
	[ $DEBUG ] && echo "*** DEBUG: $0: kernelVer is not numeric"
	exit 1
fi
echo "$kernelVer"
exit 0 
